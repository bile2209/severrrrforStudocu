...
// Can thiet cho Railway - phai dat TRUOC moi middleware khac
...
app.post('/api/verify', async (req, res) => {
    ...
    if (!key || !machine_id) return res.json({ ok: false, error: 'Thieu thong tin' });
    ...
    if (error || !data) return res.json({ ok: false, error: 'Key khong ton tai' });
    if (!data.is_active) return res.json({ ok: false, error: 'Key da bi vo hieu hoa' });

    const now = new Date();
    const expires = new Date(data.expires_at);
    if (now > expires) return res.json({ ok: false, error: 'Key da het han' });

    if (!data.machine_id) {
        await supabase.from('license_keys').update({ machine_id, last_used: now.toISOString() }).eq('key', data.key);
    } else if (data.machine_id !== machine_id) {
        return res.json({ ok: false, error: 'Key nay dang duoc dung tren may khac' });
    } else {
        await supabase.from('license_keys').update({ last_used: now.toISOString() }).eq('key', data.key);
    }

    const daysLeft = Math.ceil((expires - now) / (1000 * 60 * 60 * 24));
    return res.json({ ok: true, plan: data.plan, expires_at: data.expires_at, days_left: daysLeft, note: data.note || '' });
});
...
app.post('/api/admin/create', adminAuth, async (req, res) => {
    ...
    for (let i = 0; i < Math.min(quantity, 50); i++) {
        ...
        const { data, error } = await supabase.from('license_keys')
            .insert({ key, plan, note, expires_at: expiresAt.toISOString(), is_active: true, created_at: new Date().toISOString() })
            .select().single();
...
app.patch('/api/admin/toggle/:key', adminAuth, async (req, res) => {
    const { data: existing } = await supabase.from('license_keys').select('is_active').eq('key', req.params.key).single();
    if (!existing) return res.json({ ok: false, error: 'Khong tim thay key' });
    const { data } = await supabase.from('license_keys').update({ is_active: !existing.is_active }).eq('key', req.params.key).select().single();
    return res.json({ ok: true, key: data });
});

app.patch('/api/admin/reset/:key', adminAuth, async (req, res) => {
    const { data } = await supabase.from('license_keys').update({ machine_id: null }).eq('key', req.params.key).select().single();
    return res.json({ ok: true, key: data });
});
...
app.patch('/api/admin/extend/:key', adminAuth, async (req, res) => {
    ...
    const { data: existing } = await supabase.from('license_keys').select('expires_at').eq('key', req.params.key).single();
    if (!existing) return res.json({ ok: false, error: 'Khong tim thay key' });
    const base = new Date(existing.expires_at) > new Date() ? new Date(existing.expires_at) : new Date();
    base.setDate(base.getDate() + parseInt(days));
    const { data } = await supabase.from('license_keys').update({ expires_at: base.toISOString() }).eq('key', req.params.key).select().single();
    return res.json({ ok: true, key: data });
});
